...
from train.trainer.utils.ds_utils import get_eval_ds_config, get_train_ds_config
from train.trainer.utils.model.model_utils import create_critic_model, create_hf_model
from train.trainer.utils.module.lora import (
    convert_linear_layer_to_lora,
    make_model_gradient_checkpointing_compatible,
    only_optimize_lora_parameters,
)
from train.trainer.utils.utils import get_optimizer_grouped_parameters
...
class DeepSpeedRLHFEngine:
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        ] = self.args.per_device_mini_train_batch_size
        ...
        ds_eval_config = get_eval_ds_config(offload=self.args.offload_critic_model, stage=0)
        # Minjia: We need to set train batch size and micro batch size here to pass the sanity check of DeepSpeed engine.
        ds_eval_config[
            "train_micro_batch_size_per_gpu"
        ] = self.args.per_device_mini_train_batch_size
        ds_eval_config["train_batch_size"] = (
            self.args.per_device_mini_train_batch_size
            * torch.distributed.get_world_size()
            * self.args.gradient_accumulation_steps
        )
        ...
        # Optimizer
        AdamOptimizer = DeepSpeedCPUAdam if self.args.offload_critic_model else FusedAdam # todo
        optim_pararms = get_optimizer_grouped_parameters(
            critic_model, self.args.critic_weight_decay
        )
        optim = AdamOptimizer(
            optim_pararms, lr=self.args.critic_learning_rate, betas=(0.9, 0.95)
        )
    ...
    def _init_ref(self, actor_model_name_or_path):
        ...
        ref_model = create_hf_model(
            AutoModelForCausalLM, actor_model_name_or_path, self.tokenizer,
            ds_config
        )
    ...
    def _init_reward(self, critic_model_name_or_path):
        ...
        ds_config = get_eval_ds_config(offload=self.args.offload_reference_model, stage=zero_stage)
        ds_config[
            "train_micro_batch_size_per_gpu"
        ] = self.args.per_device_mini_train_batch_size
        ds_config["train_batch_size"] = (
        ...
        ds_eval_config = get_eval_ds_config(offload=self.args.offload_reward_model, stage=0)
        # Minjia: We need to set train batch size and micro batch size here to pass the sanity check of DeepSpeed engine.
        ds_eval_config[
            "train_micro_batch_size_per_gpu"
        ] = self.args.per_device_mini_train_batch_size
        ...
        reward_model = create_critic_model(
            ...
            num_padding_at_beginning=self.args.num_padding_at_beginning,
            rlhf_training=True,
            device = 'cpu' if self.args.offload_reward_model else 'cuda'
        )
