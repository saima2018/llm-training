...
from train.trainer.utils.utils import print_rank_0
...
class DeepSpeedPPOTrainer:
    ...
    def generate_experience(self, prompts, mask, step):
        ...
        with torch.no_grad():
            ...
            # values维度(B, L), L维度上第i个位置表示i到最后的累积奖励价值
    ...
    def train_rlhf(self, inputs):
        ...
        actor_loss = self.actor_loss_fn(
            actor_log_prob[:, start:], # actor model预测采取的动作的logprobs
            log_probs[:, start:], # actor model在experience里实际采取的动作的logprobs
            advantages,
            action_mask[:, start:],
        ) # actor loss主要值是 advantages, 乘以新旧actor logprobs的比值
