...
import deepspeed
import torch
from ppo_deepspeed import DeepSpeedPPOTrainer, DeepSpeedPPOTrainerUnsupervised
from train.trainer.rlhf.rlhf_engine import DeepSpeedRLHFEngine
...
from train.trainer.utils.data.data_utils import (
    DataCollatorRLHF,
    MiniDataset,
    create_prompt_dataset,
    get_unsupervised_data,
)
from train.trainer.utils.module.lora import convert_lora_to_linear_layer
...
def create_datasets(args, tokenizer, train_phase=3):
    ...
    prompt_train_dataset, _ = create_prompt_dataset( # 没有用到 eval_dataset
        args.local_rank,
        args.data_path,
        args.local_data_dir,
        args.data_split,
        ...
        for step, (batch_prompt, batch_unsupervised) in enumerate(
            ...
            if exp_dataset is not None:
                ...
                if args.enable_tensorboard and torch.distributed.get_rank() == 0:
                    writer.add_scalar(
                        "reward", average_reward / inner_iter, global_step=step
                    )
                    writer.add_scalar("actor_loss", actor_loss, global_step=step)
                    writer.add_scalar(
                        "actor_loss_sum", actor_loss_sum, global_step=step
                    )
                    writer.add_scalar("critic_loss", critic_loss, global_step=step)
                    writer.add_scalar(
                        "critic_loss_sum", critic_loss_sum, global_step=step
                    )
                    writer.flush()
            ...
            if step >0 and step % 30 == 0:
                save_model_ppo(rlhf_engine, args, tokenizer, step)

    if args.output_dir is not None:
        save_model_ppo(rlhf_engine, args, tokenizer)


def save_model_ppo(rlhf_engine, args, tokenizer, step):
    print_rank_0("saving model at step {}...".format(step))
