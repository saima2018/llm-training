...
class DataArguments:
    ...
    source_prefix: Optional[str] = field(
        default=None,
        metadata={
            "help": "A prefix to add before every source text. Use `|` to separate multiple prefixes in training."
        },
    )
    ...
    def init_for_training(self):  # support mixing multiple datasets
        ...
        if self.source_prefix is not None:
            ...
            prefix_list = (
                prefix_list * len(dataset_names)
                if len(prefix_list) == 1
                else prefix_list
            )
            assert len(prefix_list) == len(
                dataset_names
            ), "The number of prefixes should be either identical with datasets or 1."
        else:
        ...
        for i, name in enumerate(dataset_names):
            ...
            if "columns" in dataset_info[name]:
                ...
                dataset_attr.response_column = dataset_info[name]["columns"].get(
                    "response", None
                )
                dataset_attr.history_column = dataset_info[name]["columns"].get(
                    "history", None
                )
