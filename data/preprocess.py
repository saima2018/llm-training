...
from train.hyperparams import DataArguments
from train.trainer.utils.constants import IGNORE_INDEX
from train.trainer.utils.template import get_template
...
def preprocess_dataset(
        ...
        for i in range(len(examples["prompt"])):
            if examples["prompt"][i] and examples["response"][i]:
                ...
                dialog = prompt_template.get_dialog(
                    query, answer, examples["history"][i], prefix
                )
                yield dialog
    ...
    def preprocess_pretrain_dataset(examples):
        ...
        result = [
            [tokenizer.bos_token_id] + concatenated_ids[i : i + block_size]
    ...
    def print_supervised_dataset_example(example):
        ...
        print(
            "labels:\n{}".format(
                tokenizer.decode(
                    [
                        d if d != IGNORE_INDEX else tokenizer.pad_token_id
                        for d in example["labels"]
                    ],
    ...
    def print_pairwise_dataset_example(example):
        ...
        print(
            "accepts:\n{}".format(
                tokenizer.decode(example["accept_ids"], skip_special_tokens=False)
            )
        )
        print("reject_ids:\n{}".format(example["reject_ids"]))
        print(
            "rejects:\n{}".format(
                tokenizer.decode(example["reject_ids"], skip_special_tokens=False)
            )
        )
